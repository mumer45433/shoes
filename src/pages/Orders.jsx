...
export default function Orders() {
  ...
  useEffect(() => {
    fetch("https://shoback.vercel.app/orders")
      .then((res) => res.json())
      .then((data) => {
        console.log("Supabase Data:", data); // Check karne ke liye
        setOrders(data);
      })
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {orders.length === 0 ? (
        <div className="text-center text-gray-500 text-lg mt-20">No orders yet</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {orders.map((order) => (
            <div key={order.id} className="bg-white rounded-2xl shadow-md border hover:shadow-xl transition duration-300 flex flex-col">
              <div className="flex justify-between items-center p-4 border-b">
                <div>
                  <h2 className="font-bold text-lg">Order #{order.id}</h2>
                  <p className="text-sm text-gray-500">
                    {/* ✅ FIX: created_at use karein */}
              ...
              {/* ✅ FIX: customer_details use karein (Supabase column name) */}
              <div className="p-4 border-b text-sm text-gray-700 space-y-1">
                <p><strong>Name:</strong> {order.customer_details?.name || "N/A"}</p>
                <p><strong>Phone:</strong> {order.customer_details?.phone || "N/A"}</p>
                <p className="truncate"><strong>Address:</strong> {order.customer_details?.address || "N/A"}</p>
              </div>

              <div className="p-4 flex-grow">
                <p className="font-semibold mb-2">Items</p>
                <ul className="space-y-1 text-sm">
                  {/* ✅ FIX: order.items?.map taake undefined error na aaye */}
